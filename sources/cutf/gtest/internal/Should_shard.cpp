...
bool ShouldShard( char const *const total_shards_env, char const *const shard_index_env, bool const in_subprocess_for_death_test) {
  if (in_subprocess_for_death_test) {
	return false;
  }

  const int32_t total_shards = internal::Int32FromEnvOrDie(total_shards_env, -1);
  const int32_t shard_index = internal::Int32FromEnvOrDie(shard_index_env, -1);
